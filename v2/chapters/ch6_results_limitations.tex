...
\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{figures/depth_results.jpg}
    \caption{depth 分支：不同方位角旋转下的深度图生成结果。左侧为参考 RGB 图及深度图，右侧为不同 $\Delta\text{azimuth}$ 下模型生成的深度图。}
    \label{fig:depth_results}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{figures/canny_results.jpg}
    \caption{canny 分支：不同方位角旋转下的轮廓图生成结果。左侧为参考 RGB 图及轮廓图，右侧为不同 $\Delta\text{azimuth}$ 下模型生成的轮廓图。}
...
多步迭代推理中，以 $\Delta\text{azimuth} = 150^\circ$、单步步长 $10^\circ$ 为例，模型经过 15 步串联迭代从初始深度图逐步变换至目标视角。每步之间的变化是渐进且连贯的，验证了多步迭代策略的有效性。但由于每步的微小误差会随步数累积，步数较多时可能在物体细节（如纹理边界、细小结构）上出现逐步模糊或漂移。
...
\begin{figure}[H]
    \centering
    \textbf{[训练完成后将填入 loss 收敛曲线]}
    \caption{depth 分支与 canny 分支的训练 loss 收敛曲线（待训练完成后绘制）}
